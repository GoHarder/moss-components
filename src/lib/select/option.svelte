...
<script lang="ts">
  ...
  // MARK: Properties
  // ------------------------------------------------
  let { children, slotStart, slotEnd, slotOverline, slotHeadline, slotSupportingText, slotTrailingSupportingText, ...props }: Props = $props();

  // MARK: Variables
  // ------------------------------------------------
  let root: MdSelectOption | undefined = $state();
...
<md-select-option bind:this={root} {...props}>
  {@render children?.()}
  {@render slotStart?.()}
  {@render slotEnd?.()}
  {@render slotOverline?.()}
